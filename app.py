...
# - Ordena por fecha de cierre hasta fin de año.
# - Fichas limpias con: lugar (heurístico), tipo, dificultad, días restantes, datos clave (premio/cupos/fee).
# - Exporta CSV e ICS (para calendario).
# - Búsqueda y filtros (tipo, dificultad, rango de fechas).

import re, io, csv, time, math
...
def guess_location(text: str):
    ...
    # pistas ciudad/comunidad (muy básico)
...
st.caption("Version cloud estable. Fuentes activables, timeouts cortos, exportables y reseñas claras.")
    ...
    for it in items:
        ...
        with st.container(border=True):
            ...
            with c1:
                ...
                st.markdown(
                    f"**Fuente:** {it['source']}  •  **Tipo:** `{it['type']}`  •  **Lugar:** {it['location']}"
                )
            ...
            with c2:
                st.metric("Dificultad", f"{diff_pct}% ({it['difficulty_label_]()_
